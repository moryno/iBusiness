...
import { items, summary, columns, centerOptions } from '../data/PurchaseOrderData'
...
function MessageDiv({message}) {
  return (
    <p className="po-message">{message}</p>
  )
}
...
export const PurchaseOrder = () => {
    ...
    const save = async () => {
      ...
      try {
        ...
        resp.status === 200 ? setMessage('Purchase order has been submitted successfully.') : setMessage('Bad request')
    ...
    <main className="w-full min-h-full relative  px-3 md:px-5 py-1.5">
            ...
            <div className='po-form'>
                <div className='po-left'>
                  <div className='select-control'>
                    <label className='label-control' for="center">Cost Center:</label>
                    <select name="Center" className="select-control-input" value={costCenter} onChange={handleCostCenter} >
                      {centerOptions.map(center => (
                        <option className="select-options" value={costCenter}>{center.text}</option>
                      ))}
                    </select>
                  </div>
                  <div className='select-control'>
                    <label className='label-control' for="center">Ships to:</label>
                  ...
                  <div className='select-control'>
                    <label className='label-control' for="center">Order Amount:</label>
                    <input type='text' className="control-input" value={total} disabled/>
                  </div>
                  <div className='select-control'>
                    <label className='label-control' for="center">First Delivery Date:</label>
                    <input type='date' defaultValue={dateString} className="control-input" value={firstDeliveryDate} onChange={handleFirstdDeliveryDate} />
                  </div>
                </div>
                <div className='po-right'>
                  <div className='select-control'>
                    <label className='label-control' for="center">Supplier:</label>
                  ...
                  <div className='select-control'>
                    <label className='label-control' for="center">Order Date:</label>
                    <input type='date' defaultValue={dateString} className="control-input" value={orderDate} onChange={handleOrderDate} />
                  </div>
                  <div className='select-control'>
                    <label className='label-control' for="center">Delivery Period (Days):</label>
                ...
                <div className='select-control'>
                  <label className='label-control' for="center">Narration:</label>
                  <textarea rows="10" cols="50" style={{resize: 'none'}} type='text' className="select-control-input narration-input" value={narration} onChange={handleNarration} />
                </div>
            </div>
          </div>
        <section className="mt-5">
            <div className="po-grid mt-5">
              ...
              <DataGrid 
                  style={{height: '35vh', borderStyle:'solid', marginTop:'1rem', fontSize: '.7rem'}}
                  showBorders={true}
